...
import { StoryGenerationRequest, StoryGenerationResponse, StoryChapter, StoryBook, StoryOutlineRequest, StoryUnlockResult } from '../types';
...
export class AIStoryService {
  ...
  async generateStory(request: StoryGenerationRequest): Promise<StoryGenerationResponse> {
    try {
      ...
      if (result.chapter) {
        ...
        // Use the book's premium cover image (selected once when book was created)
        // This ensures all chapters in a book have the same cover image
        if (!result.chapter.image_url) {
            result.chapter.image_url = request.bookCoverImage || getRandomThemeImage(request.worldTheme);
        }
  ...
  async startNewBook(childId: string, worldTheme: string): Promise<StoryBook | null> {
    try {
      ...
      // Select a premium cover image for this book (selected once, used for all chapters)
      const coverImage = getRandomThemeImage(worldTheme as any);
      console.log('📚 Selected premium cover image for book:', coverImage);
  ...
  async unlockStoryForChores(childId: string, completedChoreIds: string[]): Promise<StoryUnlockResult> {
    try {
      ...
      if (childError || !child) {
        ...
        return { status: 'error', error: 'Failed to fetch child data' };
      }
      ...
      if (choresError || !chores) {
        ...
        return { status: 'error', error: 'Failed to fetch chores' };
      }
      ...
      // If no active book, signal that a new book is needed (theme selection required)
      if (!activeBook) {
        console.log('📚 No active book found, needs new book with theme selection');
        return { status: 'needs_new_book' };
      }
      ...
      if (nextChapterNum > activeBook.total_chapters) {
          ...
          const completedBookId = activeBook.id;
          ...
          // Return special status indicating book is complete and new theme selection needed
          return { 
      ...
      const request: StoryGenerationRequest = {
        ...
        chapterSynopsis,
        chapterTitle,
        bookCoverImage: activeBook.cover_image // Use the book's cover image for all chapters
      };
      ...
      if (result.success && result.chapter) {
        ...
        // Check if this was the last chapter (book is now complete)
        if (nextChapterNum >= activeBook.total_chapters) {
          console.log('📚 Final chapter generated! Book is now complete.');
          // Mark book as completed
          await supabase
            .from('story_books')
            .update({ 
          ...
          return { 
            ...
            completedBookId: activeBook.id
          };
        }
        
        return { status: 'chapter_generated', chapter: result.chapter };
      } else {
        console.warn('📚 Story generation failed:', result.error);
        return { status: 'error', error: result.error || 'Story generation failed' };
      }
    } catch (error) {
